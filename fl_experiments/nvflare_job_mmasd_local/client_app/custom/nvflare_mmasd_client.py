...
Local-only personalized FL variant: no global parameters loaded (personalization=local),
but training still runs under the NVFLARE ScatterAndGather workflow.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="FedProx mu (only used when --fl_method fedprox). Default can be set via FEDPROX_MU env var.",
    )
    parser.add_argument(
        "--warmup_rounds",
        type=int,
...
zation)
        ...
        acc = local_eval(model, test_loader, device)
        try:
            _, per_class_acc = local_eval_per_class(model, test_loader, device, 11)
            save_per_class_csv(per_class_acc, round_num, args.theme)
        except Exception:
            pass
